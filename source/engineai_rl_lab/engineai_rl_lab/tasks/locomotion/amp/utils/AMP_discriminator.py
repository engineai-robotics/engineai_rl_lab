...
class Discriminator(nn.Module):
    """Feedforward discriminator for Adversarial Motion Priors."""
    ...
    def __init__(
        ...
        feature_normalization_clip: float = 0.0,
        device: str = "cpu",
    ):
        super().__init__()
        ...
        self.feature_normalization_clip = feature_normalization_clip
        if self.feature_normalization:
            self.feature_norm = EmpiricalNormalization(shape=(self.frame_size,)).to(self.device)

    def _clip_normalized(self, x):
        if self.feature_normalization_clip > 0.0:
            return torch.clamp(x, -self.feature_normalization_clip, self.feature_normalization_clip)
        return x
    ...
    def normalize_input(self, x):
        ...
        if x.ndim == 3:
            ...
            normalized = self.feature_norm(x.reshape(-1, frame_size)).reshape(
                batch_size,
                history_length,
                frame_size,
            )
            return self._clip_normalized(normalized)
        if x.ndim == 2:
            ...
            norm_frames = [self._clip_normalized(self.feature_norm(frame)) for frame in frames]
    ...
    def forward_normalized(self, x):
        """Score an input that has already been passed through :meth:`normalize_input`.

        Splitting this out of :meth:`forward` lets the discriminator update reuse a single
        forward pass for both the adversarial loss and the gradient penalty, and makes the
        penalty gradient be taken with respect to the *normalized* input.
        """
        return self.linear_layer(self.model(x.flatten(start_dim=1))).squeeze(-1)
