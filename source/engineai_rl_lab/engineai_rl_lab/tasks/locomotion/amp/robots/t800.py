...
# Joint gains aligned with the native SDK T800 rl_walking_example configuration.
STIFFNESS_Q300H_L = 180.0
STIFFNESS_Q300H = 100.0
STIFFNESS_Q200H = 100.0
STIFFNESS_ANKLE = 40.0
STIFFNESS_TORSO = 100.0
STIFFNESS_SHOULDER_PITCH = 60.0
STIFFNESS_SHOULDER_ROLL = 50.0
STIFFNESS_SHOULDER_YAW = 50.0
STIFFNESS_ELBOW_PITCH = 60.0
STIFFNESS_ELBOW_YAW = 50.0
...
T800_ACTION_SCALE = {
    ...
    ".*_ANKLE_PITCH_.*": 0.5,
    ".*_ANKLE_ROLL_.*": 0.2,
    # The SDK walking policy excludes torso yaw; AMP controls it as its 23rd motion joint.
    ".*TORSO_YAW.*": 0.2,
...
@configclass
class T800ArticulationCfg(ArticulationCfg):
    """Articulation configuration with SDK joint-order metadata."""
...
T800_CFG = T800ArticulationCfg(
    ...
    actuators={
        ...
        "feet": ImplicitActuatorCfg(
            ...
            stiffness={
                ".*_ANKLE_PITCH.*": STIFFNESS_ANKLE,
                ".*_ANKLE_ROLL.*": STIFFNESS_ANKLE,
            },
            damping={
                ".*_ANKLE_PITCH.*": DAMPING_ANKLE,
                ".*_ANKLE_ROLL.*": DAMPING_ANKLE,
            },
            armature={
        ...
        "arms": ImplicitActuatorCfg(
            ...
            stiffness={
                ".*_SHOULDER_PITCH.*": STIFFNESS_SHOULDER_PITCH,
                ".*_SHOULDER_ROLL.*": STIFFNESS_SHOULDER_ROLL,
                ".*_SHOULDER_YAW.*": STIFFNESS_SHOULDER_YAW,
                ".*_ELBOW_PITCH.*": STIFFNESS_ELBOW_PITCH,
                ".*_ELBOW_YAW.*": STIFFNESS_ELBOW_YAW,
            },
            damping={
                ".*_SHOULDER_PITCH.*": DAMPING_ARM,
                ".*_SHOULDER_ROLL.*": DAMPING_ARM,
                ".*_SHOULDER_YAW.*": DAMPING_ARM,
                ".*_ELBOW_PITCH.*": DAMPING_ARM,
                ".*_ELBOW_YAW.*": DAMPING_ARM,
            },
            armature={
        ...
        "head": ImplicitActuatorCfg(
            ...
            stiffness={
                "J23_HEAD_PITCH": STIFFNESS_HEAD,
                "J24_HEAD_YAW": STIFFNESS_HEAD,
            },
            damping={
                "J23_HEAD_PITCH": DAMPING_HEAD,
                "J24_HEAD_YAW": DAMPING_HEAD,
            },
            armature={
