...
def add_rsl_rl_args(parser: argparse.ArgumentParser):
    ...
    arg_group.add_argument(
        ...
        default=None,
        type=_str_to_bool,
        help="Whether to resume from a checkpoint. If set without a value, resumes with the selected checkpoint.",
    )
...
def update_rsl_rl_cfg(agent_cfg: RslRlOnPolicyRunnerCfg, args_cli: argparse.Namespace):
    ...
    if args_cli.checkpoint is not None:
        ...
        agent_cfg.load_checkpoint = ".*" if checkpoint.lower() in {"resume", "latest"} else checkpoint
